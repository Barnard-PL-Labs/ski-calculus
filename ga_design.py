"""Hardware SKI genetic-algorithm engine (stages S2-S6).

Component hierarchy (bottom-up):

  ReducerCore  WHNF graph reducer over an external heap; exposes the head tag.
  EvalCore     Owns template+work heaps + a ReducerCore. Scores a candidate
               against a target truth table: per row builds `cand b1..bn T F`,
               reduces, classifies the head as T/F, accumulates matched bits.
  Evaluator    Thin Wishbone wrapper around EvalCore (host loads a candidate
               and reads the fitness back -- this is the S2 test surface).
  Generator    LFSR-driven random pure-SKI candidate generator. Writes a fresh
               preamble + random term into a template heap; every app node
               references only earlier candidate nodes (acyclic, valid pointers).
  GACore       Generator + EvalCore + counter + best-tracker. Runs autonomously:
               generate -> evaluate -> update best -> repeat, counting candidates.
  GAEngine     K parallel GACores behind one Wishbone front-end; the host sets
               the target + seed + go, then reads total candidates evaluated and
               the global best fitness. This is the throughput engine (S4/S6).

Node word: tag[31:29] (0=APP 1=S 2=K 3=I 4=T 5=F), right[27:14], left[13:0].
...
# Evaluator (S2 Wishbone wrapper) register map (byte offsets = word*4).
EV_CTRL = 0; EV_STATUS = 1; EV_FITNESS = 2; EV_TMPL_COUNT = 3
EV_CAND_ROOT = 4; EV_N_INPUTS = 5; EV_TARGET = 6; EV_MAX_STEPS = 7
EV_TMPL_BASE = 64


class Evaluator(Elaboratable):
    """Wishbone slave wrapping one EvalCore (host loads candidate, reads fitness)."""

    def __init__(self, template_depth=64, work_depth=512, max_inputs=4):
        self.core_kw = dict(template_depth=template_depth, work_depth=work_depth,
    ...
    def elaborate(self, platform):
        m = Module()
        m.submodules.core = core = EvalCore(**self.core_kw)

        tmpl_count = Signal(range(self.template_depth + 1))
        cand_root = Signal(PTR_BITS)
        n_inputs = Signal(range(self.core_kw["max_inputs"] + 1))
        target = Signal(1 << self.core_kw["max_inputs"])
        max_steps = Signal(32)
        done = Signal(init=1)
        m.d.comb += [core.tmpl_count.eq(tmpl_count), core.cand_root.eq(cand_root),
                     core.n_inputs.eq(n_inputs), core.target.eq(target),
                     core.max_steps.eq(max_steps)]
        with m.If(core.busy):
            m.d.sync += done.eq(0)
        with m.Elif(~core.start):
            m.d.sync += done.eq(1)
        ...
        is_tmpl = Signal(); tmpl_word = Signal(PTR_BITS)
        m.d.comb += [is_tmpl.eq(self.wb_adr >= EV_TMPL_BASE),
                     tmpl_word.eq(self.wb_adr - EV_TMPL_BASE)]
        # Host template writes routed to the core's template write port.
        m.d.comb += [core.t_waddr.eq(tmpl_word), core.t_wdata.eq(self.wb_dat_w),
                     core.t_wen.eq(active_write & is_tmpl & ~core.busy)]

        with m.If(active_write & ~core.busy):
            with m.Switch(self.wb_adr):
                with m.Case(EV_CTRL):
                    m.d.comb += core.start.eq(self.wb_dat_w[0])
                with m.Case(EV_TMPL_COUNT):
                    m.d.sync += tmpl_count.eq(self.wb_dat_w)
                with m.Case(EV_CAND_ROOT):
                    m.d.sync += cand_root.eq(self.wb_dat_w[:PTR_BITS])
                with m.Case(EV_N_INPUTS):
                    m.d.sync += n_inputs.eq(self.wb_dat_w)
                with m.Case(EV_TARGET):
                    m.d.sync += target.eq(self.wb_dat_w)
                with m.Case(EV_MAX_STEPS):
                    m.d.sync += max_steps.eq(self.wb_dat_w)

        with m.Switch(self.wb_adr):
            with m.Case(EV_CTRL):
                m.d.comb += self.wb_dat_r.eq(Cat(done, core.busy))
            with m.Case(EV_STATUS):
                m.d.comb += self.wb_dat_r.eq(Cat(core.overflow, core.step_limit))
            with m.Case(EV_FITNESS):
                m.d.comb += self.wb_dat_r.eq(core.fitness)
            with m.Case(EV_TMPL_COUNT):
                m.d.comb += self.wb_dat_r.eq(tmpl_count)
            with m.Case(EV_CAND_ROOT):
                m.d.comb += self.wb_dat_r.eq(cand_root)
            with m.Case(EV_N_INPUTS):
...
class GAEngine(Elaboratable):
    ...
    def __init__(self, num_cores=4, template_depth=48, work_depth=256, max_inputs=4):
